...
public class BinaryTreeUse {
	...
	public static int ceil(BinaryTreeNode<Integer> root, int k)
	{
		if (root==null)
			return -1;
		if (root.data==k)
			return root.data;
		int op1=ceil(root.left, k);
		if (op1!=-1)
			return op1;
		if (root.data>=k)
			return root.data;
	...
	public static void iterativeInorder(BinaryTreeNode<Integer> root)
	{
		...
		while (true)
		{
			...
			if (s.isEmpty())
				break;
			BinaryTreeNode<Integer> x=s.peek();
			s.pop();
	...
	public static void inorderMorrisTraversal(BinaryTreeNode<Integer> root)
	{
		if (root==null)
			return;
		while (root!=null)
		{
			...
			{
				BinaryTreeNode<Integer> bin=root.left;
				while (bin.right!=null && bin.right!=root)
					bin=bin.right;
				if (bin.right==null) {
					bin.right=root;
					root=root.left;
	...
	public static BinaryTreeNode<Integer> findPredecessor(BinaryTreeNode<Integer> root) {
	...
	public static BinaryTreeNode<Integer> greaterSumTree(BinaryTreeNode<Integer> root)
	{
		//only for BST
	...
	public static BinaryTreeNode<Integer> SumTree(BinaryTreeNode<Integer> root)
	{
		sum=0;
		return greaterSumTree(root);
	}
	
	public static void preorderMorris(BinaryTreeNode<Integer> root)
	{
		while (root!=null)
		{
			if (root.left==null)
